...
- `ncm-api`：基于 `SPlayer-Dev/ncm-api-rs`，只在 Docker 内网暴露 3000。
...
默认下载音质是 `exhigh`（极高 / 320 kbps）。可切换无损或 Hi-Res；实际可用音质取决于账号权益和歌曲版权。勾选“同时保存 LRC”时，如果歌库里还没有同名歌词，会一并保存网易云原文 LRC。
